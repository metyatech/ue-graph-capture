...
def run_uat_from_editor(editor: Path) -> Path:
    return editor.parents[6] / "Engine" / "Build" / "BatchFiles" / "RunUAT.command"


def ubt_command_from_editor(editor: Path) -> list[Path]:
    engine_dir = editor.parents[5]
    dotnet_candidates = sorted(
        (engine_dir / "Binaries" / "ThirdParty" / "DotNet").glob("*/mac-x64/dotnet"),
        reverse=True,
    )
...
def editor_candidates(association: str) -> list[EditorCandidate]:
    ...
    install_dirs.extend(
        [
            Path("/Users/Shared/Epic Games") / f"UE_{association}",
            Path("/Applications/Epic Games") / f"UE_{association}",
            Path.home() / "UnrealEngine",
        ]
    )
    return candidate_editor_from_install_dirs(
        install_dirs, "Engine/Binaries/Mac/UnrealEditor.app/Contents/MacOS/UnrealEditor"
    )
